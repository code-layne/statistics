...
\usepackage{array}
\usepackage{tabularx}
% enumitem omitted — conflicts with beamer; use \setlength{\itemsep} inline
...
\definecolor{goldacc}{HTML}{F5A623}
\definecolor{hookbg}{HTML}{D4820A}
...
% ── Helper: navy header bar with white title ──────────────────────────────────
% Usage: \navyheader{Title Text}
\newcommand{\navyheader}[1]{%
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \node[anchor=west, text=white, font=\bfseries\large,
      ...
      at ([yshift=-0.68cm]current page.north west) {#1};
  \end{tikzpicture}%
  \vspace{1.05cm}\par
}

% ── Helper: section label (small-caps style label above content) ──────────────
% Usage: \sectionlabel[color]{LABEL TEXT}
\newcommand{\sectionlabel}[2][navy]{%
  {\color{#1}\bfseries\footnotesize\MakeUppercase{#2}}\par\smallskip
}
...
\begin{frame}[t, plain]
  ...
  \begin{minipage}{0.9\textwidth}
    ...
    \vspace{0.6cm}  % <-- vertical space between sections
