%! Justifying a Claim About the Difference of Two Means Based on a CI — Unit 7, Lesson 7.7 — Exit Ticket (Answer Key)
...
\begin{enumerate}
  \item Interpret the confidence interval in context.
        \ansline{We are 95\% confident that the true difference in mean resting heart rate (runners $-$ non-runners) for all people in these populations is between $-18.4$ and $-6.2$ bpm.}
  ...
  \item A cardiologist claims that runners have a lower mean resting heart rate than
        ...
        \ansline{Yes. The entire interval is negative (below 0), so 0 is not a plausible difference. This provides convincing evidence that runners have a lower mean resting heart rate than non-runners.}
  ...
  \item If the researcher had used samples of 160 runners and 160 non-runners
        ...
        Circle one: \quad \ans{Narrower} \quad / \quad \textbf{Wider} \quad / \quad \textbf{Same} \\[4pt]
        Explain: \ansline{Larger sample sizes reduce $SE = \sqrt{s_1^2/n_1 + s_2^2/n_2}$, which decreases the margin of error and narrows the interval.}
\end{enumerate}
