%! AP Statistics — Unit 9, Lesson 9.6 — Guided Notes (Answer Key)
...
\pageheader{Unit 9, Lesson 9.6}{Guided Notes --- Answer Key}
\namedateperiod
...
\textit{Think about it:} What is the first question you should ask when you read a scenario on the AP exam? \ans{Is the variable categorical or quantitative?}
\end{hookbox}
...
\hspace{1cm} $\to$ \ans{Categorical} (categories, counts, yes/no) \quad OR \quad \ans{Quantitative} (measurements, averages)
...
\begin{tabular}{@{\hspace{0.5cm}} l l}
  ...
  \hspace{0.5cm} One proportion & $\to$ \ans{1-prop $z$ procedure} \\
  \hspace{0.5cm} Two proportions (independent) & $\to$ \ans{2-prop $z$ procedure} \\
  \hspace{0.5cm} One categorical variable, testing fit to a distribution & $\to$ \ans{Chi-square GOF test} \\
  \hspace{0.5cm} Two categorical variables (association / independence) & $\to$ \ans{Chi-square test for independence} \\
  \hspace{0.5cm} One categorical variable compared across 2+ populations & $\to$ \ans{Chi-square test for homogeneity} \\[6pt]
  \textit{If Quantitative:} & \\
  \hspace{0.5cm} One mean & $\to$ \ans{1-sample $t$ procedure} \\
  \hspace{0.5cm} Two paired means (same subjects, measured twice) & $\to$ \ans{Paired $t$ procedure} \\
  \hspace{0.5cm} Two independent means & $\to$ \ans{2-sample $t$ procedure} \\
  \hspace{0.5cm} Two quantitative variables (predict $y$ from $x$) & $\to$ \ans{$t$ procedure for slope} \\
\end{tabular}

\vspace{0.15in}
\textbf{Step 3 — CI or Significance Test?}

\hspace{1cm} \textbf{Confidence interval} $\to$ goal is to \ans{estimate} a parameter.

\hspace{1cm} \textbf{Significance test} $\to$ goal is to \ans{assess evidence against} a claim about a parameter.
\end{notesbox}
...
\hspace{0.5cm} Procedure: \ans{1-sample $t$-interval for a mean}
...
\hspace{0.5cm} Procedure: \ans{2-sample $t$-test for a difference of means}
\end{notesbox}
